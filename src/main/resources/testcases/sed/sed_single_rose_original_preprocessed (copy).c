...
# 1 "getopt.h" 1
# # 13017 "sed_single_rose_original.c" 2
...

{
  ...
  switch (rexp->type)
    ...
    case r_side_effect:
      return
 !((int)rexp->params.side_effect > 0 ? idempotent_complex_se [ params [(int)rexp->params.side_effect].se ] : idempotent_se [-(int)rexp->params.side_effect]);
...
c = (unsigned char) *p++;
                ...
                this_group_regnum = (compile_stack.stack[compile_stack.avail]).regnum;
  {
    ...
    params[right_se].op1 = this_group_regnum;
    {
      ...
      struct rexp_node * c1
        = (*inner ? rx_mk_r_concat (&rxb->rx, left, *inner) : left);
      struct rexp_node * c2
  ...
  *top_expression = alt;
  last_expression = &alt->params.pair.right;
  {
    ...
    params = (params ? ((struct re_se_params *) realloc (params,
     (paramc + 1) * sizeof (params[0]))) : ((struct re_se_params *) malloc (sizeof (params[0]))));
    if (!params)
      return REG_ESPACE;
    ++paramc;
  ...
  case rx_backtrack_point:
    {
     ...
     ; if (!new_chunk) { ret_val = 0; goto test_do_return; } } new_chunk->sp = (char *)new_chunk + sizeof (struct stack_chunk); new_chunk->bytes_left = (chunk_bytes - (backtrack_frame_bytes) - sizeof (struct stack_chunk)); new_chunk->next_chunk = backtrack_stack; backtrack_stack = new_chunk; } else (backtrack_stack->sp += (backtrack_frame_bytes)), (backtrack_stack->bytes_left -= (backtrack_frame_bytes));

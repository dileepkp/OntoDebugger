...
schedule()
{
	...
	if (prio_queue[i]->mem_count > 0)
	{
	    ...
	    prio_queue[i] = del_ele(prio_queue[i], cur_proc);//new program state instance: Priority'prio'Jobcount-- (data property)
	    return;
	}
    }
}
...
float ratio;
{
	...
	if (proc) { //most recent UpgradePrioCmd instance: upgradedJob <- based on memory of 'proc' get RDF id of Job
	    src_queue = del_ele(src_queue, proc);
	    /* append to appropriate prio queue */
	    proc->priority = prio;
	    dest_queue = append_ele(dest_queue, proc);//new program state instance: Priority'prio'Jobcount-- (data property), Priority'prio+1'Jobcount++
	}
    }
}
...
float ratio;
{
	...
	if (proc) { //most recent UnblockCmd instance: unblockedJob <- based on memory of 'proc' get RDF id of Job
	    block_queue = del_ele(block_queue, proc);
	    /* append to appropriate prio queue */
	    prio = proc->priority;
	    prio_queue[prio] = append_ele(prio_queue[prio], proc);//new program state instance: Priority'prio'Jobcount-- (data property), BlockedJobcount++
	}
    }
}
...
block_process()
{
    ...
    if (cur_proc)
    {
	block_queue = append_ele(block_queue, cur_proc);//new program state instance: BlockedJobcount++ (data property)
    }
}
...
int prio;
{
    ...
    proc->priority = prio;
    num_processes++; //new Job instance: priority 'prio', id 'proc->val', creationTime <- timestamp
    return proc;
}
...
int prio;
{
    ...
    prio_queue[prio] = append_ele(prio_queue[prio], proc);//most recent NewJob instance: hasPriority, createdJob (object prop) most recent new job instance //new program state instance: Priority'prio'Jobcount++ (data property)
}
...
int num_proc;
{
    ...
    prio_queue[prio] = queue; //new program state instance: Priority'prio'Jobcount (data property) <- 'i'; timestamp
}
...
char *argv[];
{
    ...
    {// new Command instance: timestamp
	
	
	if(command == FINISH){
	    finish_process();
	}
	else if(command == BLOCK){ //New BlockCommand instance from most recent command instance
	    block_process();
	}
	else if(command == QUANTUM_EXPIRE){
	    quantum_expire();
	}
	else if(command == UNBLOCK){ //New UnBlockCommand instance from most recent command instance
	    fscanf(stdin, "%f", &ratio);
	    unblock_process(ratio);
	}
	else if(command == UPGRADE_PRIO){ //New UpgradePrioCommand instance from most recent command instance
	    fscanf(stdin, "%d", &prio);
	    fscanf(stdin, "%f", &ratio); // most recent UpgradePrioCommand instance: hasSmallPriority 'prio', hasRatio 'ratio'
	...
	else if(command == NEW_JOB){ // NewJobCommand instance from most recent command instance
	    fscanf(stdin, "%d", &prio);
	    if (prio > MAXPRIO || prio <= 0) {
		fprintf(stdout, "** invalid priority\n");
		return;

...
if (GetKeyword(Keywords[63], curr_ptr) == 0)
{
*unit_ptr = POWER_UNIT;
}
...
#ifdef DEBUG5
printf("Unit\E0 di misura guadagno trovata amp_unit=%d\n", *amp_unit_ptr);
#endif
if (error != 0)
{
...
#ifdef DEBUG5
printf("Unit\E0 di misura trovata angle_unit=%d\n", *angle_unit_ptr);
#endif
*pp2 = *curr_ptr;
return 0;
}
...
#ifdef DEBUG5
printf("Unit\E0 di misura trovata *angle_unit_ptr=%d\n", *angle_unit_ptr);
#endif
*pp2 = *curr_ptr;
return 0;
}
...
if (GetKeyword(Keywords[52], curr_ptr) == 0)
{
*angle_unit_ptr = 1;
}
...
error = angunit(*curr_ptr, curr_ptr, angle_unit_ptr);
#ifdef DEBUG5
printf("Unit\E0 di misura angolo trovata *angle_unit_ptr=%d\n", *angle_unit_ptr);
#endif
if (error != 0)
{
...
#ifdef DEBUG5
printf("Unit\E0 di misura trovata %d", *radius_unit_ptr);
#endif
if (error != 0)
{
*radius_unit_ptr = 0;
...
if ((node_ptr->ANGLE_UNIT) == RADIANTS) {
node_ptr->THEA *= RTD;
node_ptr->PHEA *= RTD;
node_ptr->PSEA *= RTD;
node_ptr->ANGLE_UNIT = DEGREES;
...
if (((grid->PSTEP) == 0) && ((grid->QSTEP) == 0) && ((grid->ANGLE) == 0)) {
...
if (elem->GEOMETRY == RECTANGULAR_ELEM) {
pstep = grid->PSTEP;
qstep = grid->QSTEP;
pdim = elem->PDIM;
qdim = elem->QDIM;
...
#ifdef DEBUG5
printf("Unit\E0 di misura guadagno trovata gain_unit=%d\n", *gain_unit_ptr);
#endif
if (error != 0)
{
*gain_unit_ptr = DB_UNIT;
...
error = angunit(*curr_ptr, curr_ptr, angle_unit_ptr);
#ifdef DEBUG5
printf("Unit\E0 di misura angolo trovata angle_unit=%d\n", *angle_unit_ptr);
#endif
if (error != 0)
{
*angle_unit_ptr = 0;
...
if (GetKeyword(Keywords[7], curr_ptr) == 0)
{
*unit = CM_UNIT;
}

else{
if (GetKeyword(Keywords[8], curr_ptr) == 0)
{
*unit = DM_UNIT;
}

else{
if (GetKeyword(Keywords[9], curr_ptr) == 0)
{
*unit = MT_UNIT;
}
...
#ifdef DEBUG5
printf("Unit\E0 di misura trovata %d", *pqdim_unit_ptr);
#endif
if (error != 0)
{
*pqdim_unit_ptr = 0;
...
if ((group_ptr->GRPHAEXC_PTR)->OMIT_PHASE == YES) {
...
if (group_ptr->SHAPE == CIRCUL_SHAPE) {
